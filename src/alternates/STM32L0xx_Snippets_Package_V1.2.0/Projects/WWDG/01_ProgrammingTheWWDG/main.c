/**
 ...
 ===============================================================================
   ...
   - then the red LED is lit for a while (means a WWDG reset occurs)
  *    
  ******************************************************************************
  * Attention
  *
...
#define ERROR_WWDG 0x01
#define ERROR_HSI_TIMEOUT 0x02
#define ERROR_PLL_TIMEOUT 0x03
#define ERROR_CLKSWITCH_TIMEOUT 0x04

#define TEMPO_RED_LED     (2000) /* 2s */
#define TEMPO_GREEN_LED   (500) /* 500ms */
#define TEMPO_REFRESH     (10)    /* 10ms, this value can be set between 8 and 16 to have a proper WWDG refresh */
...
//  DBGMCU->APB1FZ |= DBGMCU_APB1_FZ_DBG_WWDG_STOP; /* To be able to debug */
  ...
  /* Configure WWDG */
  /* (1) set prescaler to have a rollover each about 16.5ms, set window value (about 7.5ms) */
...
void SysTick_Handler(void)
{
  ...
  static uint32_t TempoRedLed = TEMPO_RED_LED; /* 2s */
