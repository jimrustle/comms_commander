...
/**
  ...
  *         and set the Core clock (PCLK) prescaler to 16.
  * Param   None
  * Retval  None
  */
__INLINE void SetClockForADC(void)
{
  ...
  /* (2) Set APB2 clock prescaler to /16 so APB2CLK = PLLCLK/16 = 1MHz */
  RCC->APB2ENR |= RCC_APB2ENR_ADC1EN | RCC_APB2ENR_SYSCFGEN; /* (1) */
  RCC->CFGR |= RCC_CFGR_PPRE2_DIV16; /* (2) */
}
...
/**
  * Brief   This function configures the ADC to convert sequentially 3 channels
            in continuous mode.
  *         The conversion frequency is 1MHz by selecting PCLK to feed the ADC
