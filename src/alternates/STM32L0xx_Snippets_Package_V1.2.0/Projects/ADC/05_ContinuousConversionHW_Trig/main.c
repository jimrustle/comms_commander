/**
  ******************************************************************************
  * File     05_ContinuousConversionHW_Trig/main.c 
  * Author   MCD Application Team
  * Version  V1.2.0
  * Date     05-February-2016
  * Brief    This code example shows how to configure the ADC to launch continuous 
  *          conversion once an HW trigger occurs
  *
  *
  ==============================================================================
 ...
 ===============================================================================
    ...
    - At each TIM22 trigger occurence, a conversion sequence is launched, i.e the 3 channels
      are successively converted.
    - The conversion is stopped if the USER push-button is pressed and start again
      in case of a new push and synchronized by TIM22 with the scanning direction toggled.
    ...
    - The green led is toggled at each end of ADC sequence (blinking is not visible).
      ...
      the red led and the green led are switched on.
      ...
      will wait for a TIM22 external trigger.
...
/**
  ...
  *         in continuous mode.
  *         The sequence is launched by rising edge on TIM22 external trigger.
...
void ADC1_COMP_IRQHandler(void)
{
  ...
  else
  {
    if ((ADC1->ISR & ADC_ISR_OVR) != 0)  /* Check OVR has triggered the IT */
    {
      ...
      GPIOB->BRR = (1 << 4); /* switch off green led to report event occurence  */
