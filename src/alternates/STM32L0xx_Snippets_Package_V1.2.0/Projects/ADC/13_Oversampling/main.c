/**
 ...
 ===============================================================================
    - This example configures the ADC to convert 3 channels (CH4, 9 and 17) 
      to get an oversampled measure (ration 16x and 1-bit shift) 
      so max value is 0x7FF8.
    ...
    - The conversion is stopped if the USER push-button is pressed and start again
      in case of a new push with the scanning direction toggled.
...
/**
  ...
  *         in continuous mode with oversampling ration 16 and shifted by 1.
...
void ADC1_COMP_IRQHandler(void)
{
  ...
  else
  {
    if ((ADC1->ISR & ADC_ISR_OVR) != 0)  /* Check OVR has triggered the IT */
    {
      ...
      GPIOB->BSRR = (1<<(4+16)); /* Switch off green led to report it is due to overrun  */
...
void EXTI0_1_IRQHandler(void)
{
  ...
  {
    error |= ERROR_UNEXPECTED_EXT_IT; /* Report an error */  
  }
}
