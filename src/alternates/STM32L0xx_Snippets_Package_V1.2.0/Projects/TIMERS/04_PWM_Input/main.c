/**
 ...
 ===============================================================================
    - This example configures the TIM21 in order to compute the period and the  
    ...
    - To test this example, the user must applied a periodic signal on PB13.
    ...
    - The green LED is switched on till no fatal error is detected.
      In case of capture overflow, the red led is switched on till no more error
      is reported.
...
int main(void)
{
  ...
  {  
    ...
    if (error != 0)
    {
      GPIOA->BSRR = 1<<5; /* switch on red led */
    }
    else
    {
      GPIOA->BRR = 1<<5; /* switch off red led */      
    }
  }
