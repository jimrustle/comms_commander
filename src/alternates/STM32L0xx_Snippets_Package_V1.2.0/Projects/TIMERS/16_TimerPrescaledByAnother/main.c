/**
 ...
 ===============================================================================
    - This example configures the TIM21 in master mode so that it outputs 
      ...
      ITR1 is driven by Timer21.
      Timer2 is set in slave mode in external clock mode 1, so
      Timer 2 is clocked by the rising edge of the periodic Timer 21 trigger 
      signal.
...
volatile uint16_t CounterSlave = 0; // Contains the last value of the captured counter
uint16_t CounterMaster = 0; // Contains the last value of the captured counter
...
int main(void)
{
  ...
  while (1) /* Infinite loop */
  {
    ...
    if (CounterSlave != TIMy->CNT)
    {
      CounterSlave = TIMy->CNT;
