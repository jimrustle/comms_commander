/**
 ...
 ===============================================================================
  - RCC
  - TIMx
  - GPIO PA5 and PA0 for TIM2_CH1 and TIM2_ETR
  - GPIO PB4 for green LED
 ...
 ===============================================================================
    - This example configures the TIM1 in order to generate a PWM edge aligned 
      ...
      The GPIO PA5 and PA0, corresponding respectively to TIM1_CH1 and TIM1_ETR, 
      are configured as alternate function and the AFR5 is selected.
    - To test this example, the user must monitor the signal on PA5  
      (which is also connected to the red Led) and apply a signal on PA0/TIM1_ETR
      (which is connected to the USER button).
      The red led will be switched off while the USER button is pressed. 
      The red led blinks quickly while the USER button is released.
    ...
    Note : PA5 is configured to drive the led in ConfigureGPIO() in case of error 
           SystemClock_Config(). PA5 is reconfigured as alternate function 
           in ConfigureTIMxAsPWMandETR().
  *    
  ******************************************************************************
  * Attention
  *
...
/**
  ...
  *         It configures GPIO PA5 as Alternate function for TIM2_CH1
  *         and PA0 for TIM2_ETR
...
__INLINE void ConfigureTIMxAsPWMandETR(void)
{
  ...
  /* (3) Select alternate function mode on GPIOA pin 0 and 5 */
  /* (4) Select AF5 on PA5 and PA0 in AFRL for TIM2_CH1 and TIM2_ETR */
  
  RCC->APB1ENR |= RCC_APB1ENR_TIM2EN; /* (1) */
  //RCC->IOPENR |= RCC_IOPENR_GPIOAEN; /* (2) */
  GPIOA->MODER = (GPIOA->MODER & ~(GPIO_MODER_MODE0 | GPIO_MODER_MODE5)) 
               | (GPIO_MODER_MODE0_1 | GPIO_MODER_MODE5_1); /* (3) */
  GPIOA->AFR[0] |= 0x05 | (0x05 << (5*4)); /* (4) */
