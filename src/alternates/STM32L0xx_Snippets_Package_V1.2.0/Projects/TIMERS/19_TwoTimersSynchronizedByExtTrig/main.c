/**
 ...
 ===============================================================================
   ...
   - GPIO PB13 and PB14 for TIM21_CH1 (TIMx_CH1) and TIM21_CH2 (TIMx_CH2) 
 ...
 ===============================================================================
    - This example configures the TIM21 and TIM2 in order to start them 
      synchronously on an external trigger on TI1 (TIM21_CH1).
      Timer 21 is in master/slave mode with CEN used as Trigger output.
      Timer 2 must be configured in slave mode using ITR1 as internal trigger.     
      Timer2 is set in slave mode in trigger mode.
      To visualize the behaviour, TIM21_CH2 is configured in PWM mode 1 and 
      TIM2_CH1 also.
      TIM21_CH2 and TIM2_CH1 are configured with the same PWM.
      TIM21_CH1(PB13) must be wired to PA0, so a push on the USER button will 
      start both timers.
      To monitor the signals, use an oscilloscope on PB14 for TIM21_CH2 and PA5
      for TIM2_CH1.
...
__INLINE void ConfigureTIMxSynchroWithTIMy(void)
{
  ...
  /* (3) Select alternate function mode on GPIOB pin 13 and pin 14 */
