/**
 ...
 ===============================================================================
   - Solder SB16 and SB17 on the back side of the discovery board
...
__IO uint8_t stringtosend[] = "Hardware Flow Control\n";
uint8_t send = 0;
...
__INLINE void Configure_GPIO_USART1(void)
{
  ...
  GPIOA->PUPDR = (GPIOA->PUPDR & ~(GPIO_PUPDR_PUPD11))|(GPIO_PUPDR_PUPD11_0); /* (2) */
  GPIOA->PUPDR = (GPIOA->PUPDR & ~(GPIO_PUPDR_PUPD12))|(GPIO_PUPDR_PUPD12_0); /* (3) */
  GPIOA->MODER = (GPIOA->MODER & ~(GPIO_MODER_MODE9 | GPIO_MODER_MODE10 | \
               ...
               | (GPIO_MODER_MODE9_1 | GPIO_MODER_MODE10_1 | \
                  GPIO_MODER_MODE11_1 | GPIO_MODER_MODE12_1); /* (4) */
  GPIOA->AFR[1] = (GPIOA->AFR[1] &~ (0xF<<(1*4))) | (4<<(1*4)); /* (5) */
  GPIOA->AFR[1] = (GPIOA->AFR[1] &~ (0xF<<(2*4))) | (4<<(2*4)); /* (6) */
  GPIOA->AFR[1] = (GPIOA->AFR[1] &~ (0xF<<(3*4))) | (4<<(3*4)); /* (7) */
  GPIOA->AFR[1] = (GPIOA->AFR[1] &~ (0xF<<(4*4))) | (4<<(4*4)); /* (8) */
}
...
void USART1_IRQHandler(void)
{
  ...
  else if((USART1->ISR & USART_ISR_RXNE) == USART_ISR_RXNE)
  {
    ...
    {
    case 'g':
    case 'G': GPIOB->ODR ^= (1 << 4); //toggle green led
              break;
    default: break;
    }
  }
