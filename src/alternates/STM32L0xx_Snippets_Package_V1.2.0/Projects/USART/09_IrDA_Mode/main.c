/**
 ...
 ===============================================================================
   ...
   - A byte is transmit in IrDA mode
   - The IrDA mode is not compatible with UART
...
__INLINE void Configure_GPIO_USART1(void)
{
  ...
  /* (1) Select AF mode (10) on PA9, PA10 */
  /* (2) AF1 for PA9 */
  /* (3) AF1 for PA10 */
  GPIOA->MODER = (GPIOA->MODER & ~(GPIO_MODER_MODE9 | GPIO_MODER_MODE10)) | \
                 (GPIO_MODER_MODE9_1 | GPIO_MODER_MODE10_1); /* (1) */
  GPIOA->AFR[1] = (GPIOA->AFR[1] &~ (0xF<<(1*4))) | (4<<(1*4)); /* (2) */
  GPIOA->AFR[1] = (GPIOA->AFR[1] &~ (0xF<<(2*4))) | (4<<(2*4)); /* (3) */
}
...
__INLINE void Configure_USART1(void)
{
  ...
  /* Configure IT */
  /* (5) Set priority for USART1_IRQn */
  /* (6) Enable USART1_IRQn */
  NVIC_SetPriority(USART1_IRQn, 0); /* (5) */
  NVIC_EnableIRQ(USART1_IRQn); /* (6) */
}
